...
def buscar_velas(symbol, periodo="1d", intervalo="5m", tentativas=3):
    api_key = os.environ.get("TWELVEDATA_API_KEY")
    if not api_key:
        logger.error("TWELVEDATA_API_KEY nao configurada")
        return None
    ...
    for tentativa in range(1, tentativas + 1):
        try:
            ...
            if r.status_code != 200:
                logger.warning(f"[{symbol}] Tentativa {tentativa}: HTTP {r.status_code}")
                time.sleep(2)
                continue
            data = r.json()
            if data.get("status") == "error":
                logger.error(f"[{symbol}] Twelve Data erro: {data.get('message')}")
                return None
            values = data.get("values", [])
            if len(values) < 30:
                logger.warning(f"[{symbol}] Poucas velas: {len(values)}")
                time.sleep(2)
                continue
            ...
            logger.info(f"[{symbol}] {len(df)} velas (Twelve Data)")
            return df
        except Exception as e:
